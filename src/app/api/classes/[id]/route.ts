...
export async function PUT(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { grade, section, shift } = await request.json()
    if (!grade || !section || !shift) return NextResponse.json({ error: 'Missing fields' }, { status: 400 })
    ...
    if (error.code === 'P2002') return NextResponse.json({ error: 'Class already exists' }, { status: 400 })
